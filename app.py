...
def analyze_ingredients(ingredients_list, reference_dict):
    flagged = []
    for ingredient in ingredients_list:
        norm_ingredient = normalize_text(ingredient)
        for ref in reference_dict:
            norm_ref = normalize_text(ref)
            if norm_ref in norm_ingredient or is_close_match(norm_ingredient, norm_ref):
                flagged.append({"ingredient": ingredient, "info": reference_dict[ref]})
                break
    return flagged
...
if uploaded_file:
    ...
    st.write("**Beneficial Ingredients Matched:**")
    st.write(good)
